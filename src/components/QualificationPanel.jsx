  ...
  return (
    <div className={`rounded-2xl border p-4 ${
      ...
      <div className="flex items-center justify-between mb-3">
        ...
        {remaining > 0 ? (
          <span className="text-xs px-2.5 py-1 rounded-full bg-draw/10 text-draw font-semibold">
            {remaining} pool match{remaining !== 1 ? 'es' : ''} remaining
          </span>
        ) : (
          <span className="text-xs px-2.5 py-1 rounded-full bg-win/10 text-win font-semibold">
      ...
      <div className="space-y-3">
        {gamePools.map(pool => {
          ...
          return (
            <div key={pool.id} className={`rounded-xl p-3 ${darkMode ? 'bg-white/[0.02]' : 'bg-gray-50'}`}>
              ...
              {poolQualified.length > 0 ? (
                <div className="flex flex-wrap gap-2">
                  {poolQualified.map(q => {
                    ...
                    return (
                      <div key={q.teamId} className="flex items-center gap-1.5 px-2.5 py-1 rounded-full bg-win/10 border border-win/20 transition-all duration-200 hover:bg-win/15">
                        <TeamLogo team={team} size={18} />
                        <span className="text-xs font-semibold text-win">{team?.shortCode}</span>
                        <span className={`text-[10px] ${darkMode ? 'text-gray-500' : 'text-gray-400'}`}>({q.rank}{q.rank === 1 ? 'st' : 'nd'})</span>
                        {q.manual && <span className="text-[10px] text-draw font-semibold">M</span>}
                      </div>
                    );
                  })}
                </div>
