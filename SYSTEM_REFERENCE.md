...
1. **~~Settings.jsx points reference table is WRONG~~** (FIXED, FIX-004): Now shows correct bye=4pts and accurate tiebreaker rules.

2. **~~Settings.jsx tiebreaker display is WRONG~~** (FIXED, FIX-004): Now shows correct 4-step hierarchy with "No H2H, No alphabetical" note.
...
### Bug 1: ~~Settings Page Incorrect Scoring Display~~ (FIXED)
**Description:** Settings.jsx showed "Bye (present): 2 pts (walkover)" and incorrect tiebreaker rules (Head-to-head, Alphabetical)
**Root cause:** Hardcoded wrong values in the Settings component's JSX
**Files:** `src/pages/Settings.jsx`
**Severity:** ~~Low~~ Fixed (2026-03-29, FIX-004)

### Bug 2: ~~Points Breakdown Popover May Show Wrong Individual Sport Points~~ (FIXED)
**Description:** The detailed breakdown for individual sports in PointsBreakdownPopover didn't enforce `maxParticipationCap`, so if a team exceeded the cap, the breakdown total could exceed the actual standings total.
**Root cause:** `getTeamIndividualGameBreakdown()` in `breakdownData.js` added participation points per athlete without checking the game-level cap
**Files:** `src/utils/breakdownData.js` (function `getTeamIndividualGameBreakdown`), `src/components/PointsBreakdownPopover.jsx`
**Severity:** ~~Medium~~ Fixed (2026-03-29, FIX-005)
...
|--------|-------------|------------|--------|-------------|------------|------------|----------------|
| BUG-001 | Settings page shows bye=2pts instead of 4pts | Hardcoded wrong value in JSX | Fixed | Changed '2 pts' → '4 pts' and detail to '4 walkover (equal to win)' | 2026-03-25 | 2026-03-29 | src/pages/Settings.jsx |
| BUG-002 | Settings page shows wrong tiebreaker rules | Hardcoded "H2H, Alphabetical" instead of actual rules | Fixed | Replaced 3-item list with correct 4-item tiebreaker hierarchy + "No H2H, No alphabetical" note | 2026-03-25 | 2026-03-29 | src/pages/Settings.jsx |
| BUG-003 | Individual points breakdown ignores participation cap | breakdownData.js doesn't enforce maxParticipationCap | Fixed | Added participationCount tracker and cap check in getTeamIndividualGameBreakdown; added 'capped' flag per athlete and cap info display in popover | 2026-03-25 | 2026-03-29 | src/utils/breakdownData.js, src/components/PointsBreakdownPopover.jsx |
...
| BUG-006 | Clicking any points number on leaderboard crashes app (ErrorBoundary) | `getTeamIndividualGameBreakdown` in breakdownData.js returned `config` variable that was scoped inside a for-loop, causing ReferenceError at return statement | Fixed | Removed out-of-scope `config` from return object; cleaned up unused destructuring in PointsBreakdownPopover; added null guards on `game` object access and `GameHeader` component; added error logging to ErrorBoundary | 2026-03-29 | 2026-03-29 | src/utils/breakdownData.js, src/components/PointsBreakdownPopover.jsx, src/components/ErrorBoundary.jsx |
| BUG-007 | Individual games have no way to mark as completed; no champion/top team shown on leaderboard like team and lobby games | No `individualGameStatus` state existed; Dashboard never computed champion for individual games; no Finish/Reopen buttons in IndividualGameView | Fixed | Added `individualGameStatus` to state/save/load/import; added COMPLETE/REOPEN reducer actions; Dashboard now shows champion and Completed badge for individual games; IndividualGameView shows Finish/Reopen buttons and podium | 2026-03-29 | 2026-03-29 | src/context/TournamentContext.jsx, src/pages/Dashboard.jsx, src/pages/IndividualGameView.jsx |
...
| FIX-002 | Clicking any points number on leaderboard crashes app with "Something went wrong" (BUG-006). | `getTeamIndividualGameBreakdown()` in `breakdownData.js` returned `{ categories, subtotal, config }` but `config` was declared inside a `for` loop body (line 163), making it out of scope at the return statement (line 218). This caused `ReferenceError: config is not defined` every time the total breakdown popover opened for any team with individual game data. | 1. Removed `config` from the return object in `getTeamIndividualGameBreakdown()` (it was unused by callers). 2. Cleaned up corresponding destructuring in `PointsBreakdownPopover.jsx`. 3. Added null guards on `breakdown.game` access (optional chaining). 4. Added null check in `GameHeader` component. 5. Enabled error logging in `ErrorBoundary.componentDidCatch`. | `breakdownData.js` line 218: `{ categories, subtotal, config }` → `{ categories, subtotal }`. `PointsBreakdownPopover.jsx`: removed `config` destructuring, added `?.` on `breakdown.game.emoji`/`name` in 3 title strings, added `if (!game) return null` in `GameHeader`. `ErrorBoundary.jsx`: `componentDidCatch` now logs error + componentStack. | 2026-03-29 | All popover types tested (total, KO, individual, lobby, wins, losses, draws, byes). No console errors. No regressions. |
| FIX-004 | Settings page showed Bye (present) as 2pts and wrong tiebreaker rules (BUG-001, BUG-002). | Hardcoded wrong values in Settings.jsx JSX: bye showed '2 pts' instead of '4 pts', tiebreaker listed "Most wins → Head-to-head → Alphabetical" instead of actual "Points → Wins+Golds → Silvers → Bronzes" hierarchy. | 1. Changed bye points from '2 pts' to '4 pts' and detail to '4 walkover (equal to win)'. 2. Replaced 3-item wrong tiebreaker list with correct 4-item list + italic note "No head-to-head. No alphabetical. Remaining ties share the same rank." | `Settings.jsx` line 417: `points: '2 pts', detail: 'Walkover points'` → `points: '4 pts', detail: '4 walkover (equal to win)'`. Lines 433-437: replaced 3 `<li>` items with 4 correct ones + `<p>` disclaimer. | 2026-03-29 | Settings page renders correctly. Scoring logic unchanged (display-only fix). All 56 tests pass. |
| FIX-005 | Points breakdown popover showed wrong individual sport totals when participation cap active (BUG-003). | `getTeamIndividualGameBreakdown()` in `breakdownData.js` awarded participation points to every athlete without tracking a running count against `maxParticipationCap`. The real scoring function `getIndividualPointsForTeam()` enforces the cap, causing mismatch between popover and standings. | 1. Added `participationCount` tracker and `cap` from `resolvePointsConfig()` at game level. 2. Each athlete's participation point now checked against cap; if exceeded, `participationPts=0` and `capped=true`. 3. Return object now includes `participationCap` and `participationCount`. 4. Updated both popover render paths (GameSection + getGameBreakdownContent) to show "cap" badge on capped athletes and a footer showing cap usage. | `breakdownData.js` `getTeamIndividualGameBreakdown()`: added `gameConfig`, `cap`, `participationCount` variables; replaced unconditional `participationPts = config.participation` with cap-checked logic; added `capped` field to athlete detail; return now includes `participationCap`, `participationCount`. `PointsBreakdownPopover.jsx`: two render blocks (GameSection individual + getGameBreakdownContent individual) updated to show `a.capped` badge and cap usage footer. | 2026-03-29 | All 56 tests pass. Breakdown popover tested for total, individual, per-game views. No regressions. Breakdown totals now match standings. |
